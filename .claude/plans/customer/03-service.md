...
**Audit note**: `created_by` / `updated_by` are stamped automatically by `CustomerObserver`.
The service never sets them manually.

**Import/Export**: Deliberately excluded from this module. Will be added as a separate
feature later. Placeholder comments mark where they would go.
...
| Method           | Description                                             |
|------------------|---------------------------------------------------------|
| `list()`         | Paginated list — role-priority scoped (admin/mgr/sales) |
| `create()`       | Create customer; Observer stamps audit fields           |
| `update()`       | Update customer; Observer stamps updated_by             |
| `changeStatus()` | Transition status enum; Observer stamps updated_by      |
| `assign()`       | Set or clear assigned_to; Observer stamps updated_by    |
| `delete()`       | Soft-delete                                             |
