...
| subtotal | decimal(12,2) unsigned | No | 0.00 | Sum of order_lines.line_total |
...
| grand_total | decimal(12,2) unsigned | No | 0.00 | subtotal + fees + shipping |
...
### Snapshot rules
| Payment method | Billing snapshot | Shipping snapshot |
|----------------|-----------------|-------------------|
| `stripe_card` (online) | Required — Stripe card-not-present needs billing | Required if carrier delivery |
| `stripe_terminal` | NULL — card-present, no manual entry | Required if carrier delivery |
| `cash` | NULL | Required if carrier delivery |
| `stripe_checkout` | NULL — Stripe hosted page handles billing | Required if carrier delivery |
| `cheque` | NULL | Required if carrier delivery |
| Any method + in-store pickup | NULL | NULL |

> Both snapshot groups are **immutable** after order creation. For re-ship after RTS, use `shipments.customer_address_id` FK — never mutate the snapshot.
...
| inventory_serial_id | foreignId | Yes | null | FK → inventory_serials.id — NULL when back-ordered (stock not yet in). **unique** when set — no duplicate serials across lines |
| unit_price | decimal(10,2) unsigned | No | — | Price at order time |
| tax_rate | decimal(6,4) unsigned | No | 0.0000 | e.g. `0.0825` = 8.25% |
| tax_amount | decimal(10,2) unsigned | No | 0.00 | unit_price × tax_rate |
| line_total | decimal(10,2) unsigned | No | — | unit_price + tax_amount |
| created_at | timestamp | Yes | — | Auto |
| updated_at | timestamp | Yes | — | Auto |
...
> `customer_addresses` has no FK from `orders` — snapshots are plain string columns copied at order creation, no DB constraint. `customer_addresses` dependency belongs to `shipment/01-schema.md` only.
> `products` removed — `order_lines.product_id` not in spec. Reach product via `inventory_serials.product_id`.
...
| `back_ordered` | Set at creation when no serial available (stock not in). Payment tracked separately via `payment_status` — may be paid or unpaid | no |
...
- Generated in `OrderService::generateNumber()`
...
Order hasMany OrderFees
Order hasMany Payments
Order hasMany Shipments (polymorphic)
...
OrderLine belongsTo InventorySerial
OrderLine hasMany Complaints

OrderFee belongsTo Order
```
