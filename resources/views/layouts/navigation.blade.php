        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @canany(['suppliers.viewAny', 'purchase-orders.viewAny', 'pipeline.viewAny'])
                        <div class="relative flex items-stretch" x-data="{ open: false }" @click.outside="open = false">
                            <button @click="open = !open" class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->routeIs('suppliers.*') || request()->routeIs('purchase-orders.*') || request()->routeIs('pipeline.*') || request()->routeIs('po-returns.*') ? 'border-indigo-400 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} text-sm font-medium leading-5 focus:outline-none transition duration-150 ease-in-out">
                                {{ __('Procurement') }}
                                <svg class="ms-1 fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                </svg>
                            ...
                            <div x-show="open"
                                ...
                                @endcan
                                @can('purchase-orders.viewAny')
                                    <x-dropdown-link :href="route('purchase-orders.index')">{{ __('Purchase Orders') }}</x-dropdown-link>
                                @endcan
                                @can('pipeline.viewAny')
                                    <x-dropdown-link :href="route('pipeline.queue')">{{ __('Pipeline') }}</x-dropdown-link>
                                @endcan
                                @can('purchase-orders.viewAny')
                                    <x-dropdown-link :href="route('po-returns.index')">{{ __('PO Returns') }}</x-dropdown-link>
                                @endcan
                            </div>
                        </div>
                    @endcanany
