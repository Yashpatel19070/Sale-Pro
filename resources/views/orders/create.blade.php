        ...
        <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('orders.store') }}"
                  x-data="{
                      ...
                      lines: [{ product_listing_id: '', unit_price: 0, tax_amount: 0, sku: '', stock: '', fees: [] }],
                      shipping: 0,
                      taxTimer: null,
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                        ...
                        <div>
                            ...
                            <select name="payment_method" required class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm">
                                @foreach($paymentMethods as $m)<option value="{{ $m->value }}">{{ $m->label() }}</option>@endforeach
                            </select>
                        </div>
                        <div>
                ...
                <div class="rounded-lg bg-gray-50 px-5 py-4">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="submit" class="rounded-md bg-indigo-600 px-5 py-2 text-sm font-medium text-white hover:bg-indigo-700">Create Order</button>
                    </div>
                </div>
            </form>
        </div>
