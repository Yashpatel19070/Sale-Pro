<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                <div class="mb-4 rounded-md bg-green-100 px-4 py-3 text-green-800">{{ session('success') }}</div>
            @endif
            ...
            <form method="GET" action="{{ route('orders.index') }}" class="mb-6 flex flex-wrap gap-3">
                ...
                <button type="submit"
                        class="rounded-md bg-gray-800 px-4 py-2 text-sm text-white hover:bg-gray-700">Filter</button>
                <a href="{{ route('orders.index') }}"
                   class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Clear</a>
            </form>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @foreach($orders as $order)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-500">{{ $order->source->label() }}</td>
