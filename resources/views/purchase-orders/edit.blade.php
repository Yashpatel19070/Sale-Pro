<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Purchase Order — {{ $purchaseOrder->po_number }}</h2>
        ...
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('purchase-orders.update', $purchaseOrder) }}"
                  x-data="{
                      ...
                      addLine() {
                          this.lines.push({ product_id: '', description: '', qty_ordered: 1, unit_cost: 0, tax_rate: 0, line_total: 0, qty_on_hand_snapshot: 0 });
                      },
                      removeLine(i) {
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                <template x-for="(line, index) in lines" :key="index">
                                    <tr>
                                        <td class="px-4 py-3">
                                            <input type="hidden"
                                                   :name="`lines[${index}][qty_on_hand_snapshot]`"
                                                   :value="line.qty_on_hand_snapshot ?? 0" />
