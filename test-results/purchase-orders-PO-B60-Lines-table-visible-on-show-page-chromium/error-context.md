...
- Name: purchase-orders.spec.ts >> PO-B60: Lines table visible on show page
- Location: tests/Browser/purchase-orders.spec.ts:228:1
...
Error: strict mode violation: locator('table') resolved to 2 elements:
    1) <table class="min-w-full divide-y divide-gray-200">…</table> aka getByText('PO Number Type Supplier Status Lines Created By Created No purchase orders')
    2) <table>…</table> aka locator('table').filter({ hasText: 'IDDateMethodURLIPFilter data' })
